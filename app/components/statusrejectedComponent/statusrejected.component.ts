...
@Component({
    selector: 'bh-statusrejected',
    templateUrl: './statusrejected.template.html',
    styleUrls: ['./statusrejected.component.scss']
})



export class statusrejectedComponent implements OnInit {
    ...
    redirectToEditUploadFile(id, currentTab?) {
        ...
        this.dmStatus.getFileByUuid(filterObj).then((res) => {
            let businessDept = res.local.res[0].businessDepartment || null;
    ...
    _getTableData(pageNumber?, pageSize?, filterValue?) {
        this.dmStatus.getMyFiles(pageSize, 'rejected', pageNumber,  this.filterObj , { timestamp: -1 })
                .then((res) => {
                    ...
                    if(this.result2.length == 0){
                        ...
                        this.dmStatus.getMyFiles(pageSize, 'rejected', pageNumber+1,  this.filterObj , { timestamp: -1 })
                        .then((res3) => {
